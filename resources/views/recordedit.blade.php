...
@if($temperaturesOnSelectedDate->count() > 0)
      <div class="flex flex-col mb-10 lg:items-start items-center" _msthidden="3">
        ...
        <div class="flex-grow" _msthidden="3">
          ...
          @foreach ($temperaturesOnSelectedDate as $index => $temperature)
           ...
           @if($lastTraining)
          <div>
          <p class="text-gray-900 font-bold text-xl px-3">
                  今日行ったトレーニングは 
                  @if(!empty($communicationData) && is_array($communicationData) && count($communicationData) > 0)
                      <span class="text-gray-900 font-bold text-xl px-3">コミュニケーション</span>
                  @endif
                  
                  @if(!empty($exerciseData) && is_array($exerciseData) && count($exerciseData) > 0)
                      <span class="text-gray-900 font-bold text-xl px-3">運動</span>
                  @endif
                  
                  @if(!empty($reading_writingData) && is_array($reading_writingData) && count($reading_writingData) > 0)
                      <span class="text-gray-900 font-bold text-xl px-3">読み書き</span>
                  @endif
                  
                  @if(!empty($calculationData) && is_array($calculationData) && count($calculationData) > 0)
                      <span class="text-gray-900 font-bold text-xl px-3">計算</span>
                  @endif
                  
                  @if(!empty($homeworkData) && is_array($homeworkData) && count($homeworkData) > 0)
                      <span class="text-gray-900 font-bold text-xl px-3">宿題</span>
                  @endif
                  
                  @if(!empty($shoppingData) && is_array($shoppingData) && count($shoppingData) > 0)
                      <span class="text-gray-900 font-bold text-xl px-3">買い物</span>
                  @endif
                  
                  @if(!empty($training_otherData) && is_array($training_otherData) && count($training_otherData) > 0)
                      <span class="text-gray-900 font-bold text-xl px-3">その他</span>
                  @endif
                  <span class="text-gray-900 font-bold text-xl px-3">でした。</span>
              </p>
            </div>
            @endif    
    ...
    <!-- @if($lastOptions)
      <div class="flex flex-col mb-10 lg:items-start items-center" _msthidden="3">
        ...
        <div class="flex-grow p-4" _msthidden="3">
          <h2 class="text-gray-900 text-lg title-font font-medium mb-3" _msttexthash="204971" _msthidden="1" _msthash="743">title</h2>
            <div class="flex justify-around text-left items-start">
              
              @php
                  $item1 = json_decode($lastOptions->item1);
                  $item2 = json_decode($lastOptions->item2);
                  $item3 = json_decode($lastOptions->item3);
                  $item4 = json_decode($lastOptions->item4);
                  $item5 = json_decode($lastOptions->item5);
              @endphp
              
              @if(!empty($item1) && is_array($item1) && count($item1) > 0)
                  <p class="text-gray-900 font-bold text-xl px-3">{{ $lastOptions->item1 }}</p>
              @endif
              
              @if(!empty($item2) && is_array($item2) && count($item2) > 0)
                  <p class="text-gray-900 font-bold text-xl px-3">{{ $lastOptions->item2 }}</p>
              @endif
              
              @if(!empty($item3) && is_array($item3) && count($item3) > 0)
                  <p class="text-gray-900 font-bold text-xl px-3">{{ $lastOptions->item3 }}</p>
              @endif

              @if(!empty($item4) && is_array($item4) && count($item4) > 0)
                  <p class="text-gray-900 font-bold text-xl px-3">{{ $lastOptions->item4 }}</p>
              @endif

              @if(!empty($item5) && is_array($item5) && count($item5) > 0)
                  <p class="text-gray-900 font-bold text-xl px-3">{{ $lastOptions->item5 }}</p>
              @endif
          </div>
            @if($lastOptions->bikou !== null)
                <p class="text-gray-900 font-bold text-xl px-3">{{ $lastOptions->bikou }}</p>
